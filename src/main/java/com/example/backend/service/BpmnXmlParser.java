...
public class BpmnXmlParser {
    ...
    private List<FlowRequestDTO> readFlows(Document doc, List<ActivityRequestDTO> activities) {
        ...
        // Track (sourceRef, targetRef) pairs so message flows don't
        // duplicate a sequence flow that already wires the same activities.
        ...
        // Also accept bpmn:messageFlow edges. Strict BPMN reserves message
        // flows for inter-pool messages, but the visual modeler in this
        // product sometimes emits one when the admin connects activities
        // across lanes (different departments inside the same process).
        // For our domain, lanes are purely organisational, so these
        // cross-lane links are real control-flow edges and the operator
        // pipeline depends on us honouring them. Treat them as LINEAR
        // sequence flows unless an equivalent edge is already there.
        NodeList mf = doc.getElementsByTagNameNS(NS_BPMN, "messageFlow");
        for (int i = 0; i < mf.getLength(); i++) {
            Element flow = (Element) mf.item(i);
            String source = flow.getAttribute("sourceRef");
            String target = flow.getAttribute("targetRef");
            if (isBlank(source) || isBlank(target)) continue;
            if (seenEdges.contains(source + "->" + target)) continue;
            flows.add(FlowRequestDTO.builder()
                    .sourceRef(source)
                    .targetRef(target)
                    .type("LINEAR")
